# How claude-recall Uses Hooks: A Lifecycle-Driven Architecture
...
claude-recall is fundamentally a **hook-driven system**. Every piece of functionality happens in response to lifecycle events:
...
**Key insight:** claude-recall doesn't interrupt or modify Claude Code's behavior. It observes from the outside and provides value through lifecycle hooks.
...
claude-recall had several architectural constraints:
...
claude-recall uses lifecycle hook scripts across 5 lifecycle events. SessionStart runs 3 hooks in sequence: smart-install, worker-service start, and context-hook.
...
- ✅ Configurable observation count via `CLAUDE_RECALL_CONTEXT_OBSERVATIONS`

**Output format:**
```markdown
# [claude-recall] recent context
...
**Source:** `src/hooks/context-hook.ts` → `extension/runtime/context-hook.js`
...
**Source:** `src/hooks/new-hook.ts` → `extension/runtime/new-hook.js`
...
**Source:** `src/hooks/save-hook.ts` → `extension/runtime/save-hook.js`
...
**Source:** `src/hooks/summary-hook.ts` → `extension/runtime/summary-hook.js`
...
**Source:** `src/hooks/cleanup-hook.ts` → `extension/runtime/cleanup-hook.js`
...
<AccordionGroup>
  ...
  <Accordion title="Observations not captured">
    ...
    **Debugging:**
    1. Check database: `sqlite3 ~/.claude-recall/claude-recall.db "SELECT * FROM observation_queue"`
...
echo '{
  ...
  "source": "startup"
}' | node extension/runtime/context-hook.js
...
}' | node extension/runtime/save-hook.js
...
**Privacy guarantees:**
- All data stored locally in `~/.claude-recall/claude-recall.db`
...
*The hook-driven architecture enables claude-recall to be both powerful and invisible. Users never notice the memory system working - it just makes Claude smarter over time.*
